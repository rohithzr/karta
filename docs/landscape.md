...
| **Hippocampus: Efficient and Scalable Memory Module for Agentic AI** *(arxiv 2602.13594, Feb 2026)* | Replaces dense vector DBs and knowledge graphs with compact binary signatures for semantic search and lossless token-ID streams for exact reconstruction; Dynamic Wavelet Matrix (DWM) co-indexes both streams and searches within the compressed domain — no decompression step; linear scalability relative to memory size | Up to 31× latency reduction, 14× fewer tokens per query; competitive accuracy on LoCoMo and LongMemEval (exact scores in paper) | [arxiv 2602.13594](https://arxiv.org/abs/2602.13594) |
| **GAM: Hierarchical Graph-based Agentic Memory** *(arxiv 2604.12285, Apr 2026)* | Separates memory encoding from consolidation: maintains ongoing dialogue in an event progression graph; integrates into a topic associative network only upon semantic shifts; graph-guided multi-factor retrieval; addresses the stability-plasticity dilemma in incremental memory | Consistently outperforms SOTA baselines on LoCoMo and LongDialQA; specific scores in paper | [arxiv 2604.12285](https://arxiv.org/abs/2604.12285) |
| **HMO: Hierarchical Memory Orchestration for Personalized Persistent Agents** *(arxiv 2604.01670, Apr 2026)* | Three-tier hierarchy: compact primary cache (recent + pivotal memories) + high-priority secondary layer + global archive; user-centric relevance redistribution surfaces historical knowledge while keeping active search space efficient; couples memories with evolving user profile; deployed in OpenClaw ecosystem | SOTA claimed; tested on multiple benchmarks (specific numbers in paper); real-world deployment reported | [arxiv 2604.01670](https://arxiv.org/abs/2604.01670) |
| **Cognis: Context-Aware Memory for Conversational AI Agents** *(arxiv 2604.19771, Apr 2026)* | Dual-store backend (OpenSearch BM25 + Matryoshka vector similarity) fused via Reciprocal Rank Fusion; context-aware ingestion retrieves existing memories before extraction for intelligent version tracking; temporal boosting for time-sensitive queries; BGE-2 cross-encoder reranker; open-source, deployed in production | Claims SOTA on both LoCoMo and LongMemEval across 8 answer generation models (specific scores in 30-page paper) | [arxiv 2604.19771](https://arxiv.org/abs/2604.19771) |
| **MemTrace: Error Tracing and Attribution in LLM Memory Systems** *(arxiv 2605.28732, May 2026)* | Transforms memory pipelines into executable memory evolution graphs; iterative subgraph tracing isolates root causes (information loss, retrieval misalignment) of memory failures; reveals failures are systematic, not random; closed-loop correction applies attribution signals to fix identified faults; MemTraceBench tests Long-Context, RAG, Mem0, EverMemOS | End-task performance improvement up to 7.62% via targeted fault correction | [arxiv 2605.28732](https://arxiv.org/abs/2605.28732) |
| **User as Code: Executable Memory for Personalized Agents** *(arxiv 2606.16707, Jun 2026)* | Represents user model as typed Python objects (state) + ordinary Python functions (rules) — "a living software project"; append-only log preserves all facts, periodically condensed into executable typed code; addresses retrieval-based failure modes on contradictions, aggregation, and rule enforcement | LoCoMo: 78.8% recall (matches full-context and prior memory systems); aggregate queries ("how many X?") 99% accuracy vs 6–43% for retrieval-based approaches | [arxiv 2606.16707](https://arxiv.org/abs/2606.16707) |
| **AgenticSTS: Bounded-Memory Testbed for Long-Horizon LLM Agents** *(arxiv 2607.02255, Jul 2026)* | Long-horizon bounded-memory evaluation using Slay the Spire 2 (deck-building game requiring 100s of decisions); "bounded-memory contract" — each decision assembled from typed retrieval (no raw transcript append); enables per-layer ablation; frontier models score 0 wins at lowest difficulty; human win rate ~16%; 298 completed trajectories + frozen memory snapshots released | Claude A0 baseline: 3/10 wins; with skill memory layers: 6/10 — directional, not statistically decisive | [arxiv 2607.02255](https://arxiv.org/abs/2607.02255) |
| **ByteRover: Agent-Native Memory Through LLM-Curated Hierarchical Context** *(arxiv 2604.01599, Apr 2026)* | Agent-centric design: same LLM handling task reasoning also curates and retrieves knowledge, eliminating semantic misalignment between capture intent and storage; hierarchical Context Tree (domain→topic→subtopic→entry) with explicit relationships and source tracking; 5-tier progressive retrieval resolves most queries sub-100ms without LLM calls, escalates to agentic reasoning for novel questions; Adaptive Knowledge Lifecycle (importance ratings, maturity stages, recency adjustment); zero external infrastructure — no vector DB, no embedding service, all knowledge as local markdown files | LoCoMo: 96.1% (v2.1.5, self-published); LongMemEval-S: 92.8% (v2.1.5, self-published, Gemini 3 Flash curation + Gemini 3.1 Pro justification) | [arxiv 2604.01599](https://arxiv.org/abs/2604.01599) |
...
| 2026-07-08 | **Added 7 new papers** backfilled from Apr–Jul 2026 sweep: Hippocampus (2602.13594 — Dynamic Wavelet Matrix, 31× latency reduction, 14× fewer tokens), GAM (2604.12285 — hierarchical graph-based agentic memory, event progression + topic associative network), HMO (2604.01670 — 3-tier hierarchical memory orchestration, OpenClaw deployed), Cognis (2604.19771 — context-aware memory, BM25+Matryoshka+RRF+BGE-2, SOTA claimed on LoCoMo + LongMemEval across 8 models, OSS in production), MemTrace (2605.28732 — error tracing via executable memory evolution graphs, +7.62% performance via fault attribution, tests Mem0 + EverMemOS), User as Code (2606.16707 — executable Python typed memory, 78.8% LoCoMo recall, 99% aggregate queries vs 6–43% for retrieval-based), AgenticSTS (2607.02255 — Slay the Spire 2 bounded-memory testbed, 298 trajectories, Claude A0 baseline 3/10 → 6/10 with skill memory). | [arxiv 2602.13594](https://arxiv.org/abs/2602.13594), [arxiv 2604.12285](https://arxiv.org/abs/2604.12285), [arxiv 2604.01670](https://arxiv.org/abs/2604.01670), [arxiv 2604.19771](https://arxiv.org/abs/2604.19771), [arxiv 2605.28732](https://arxiv.org/abs/2605.28732), [arxiv 2606.16707](https://arxiv.org/abs/2606.16707), [arxiv 2607.02255](https://arxiv.org/abs/2607.02255) |
| 2026-07-09 | **MemOS v2.0.23 released** (Jul 9, 2026): bug fixes for system parser initialization, JSON response handling, and cube sharing validation; adds configurable CJK keyword tokenization, embedding input routing by usage type, and a data export endpoint for memos-local; fixes OpenClaw plugin registration blocking, skill crystallization retries, and SQLite native binding quarantine. | [github.com/MemTensor/MemOS/releases](https://github.com/MemTensor/MemOS/releases) |
| 2026-07-09 | **Added ByteRover paper** (arxiv 2604.01599, Apr 2, 2026) to papers table — arxiv paper for the ByteRover system already tracked in the matrix; LLM-curated hierarchical Context Tree; LoCoMo 96.1% and LongMemEval-S 92.8% (v2.1.5, self-published). | [arxiv 2604.01599](https://arxiv.org/abs/2604.01599) |
| 2026-07-10 | No significant changes detected in tracked sources. EverMemOS LanceDB FTS crash + disk-bloat fix merged (Jul 10, github.com/EverMind-AI/EverMemOS). All other findings from today's research sweep already covered by prior runs. | Scheduled run |
